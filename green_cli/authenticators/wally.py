...
class WallyAuthenticator(MnemonicOnDisk, HardwareDevice):
    ...
    # WallyAuthenticator supports AE and traditional EC signatures.
    # Can tweak this to easily test GDK support for other hww configurations:
    # 0 = No AE support, 1 = AE and EC, 2 = AE only.
    AE_SUPPORT_LEVEL = 1

    """Uses libwally device to authenticate"""
    ...
    def hw_device(self):
        return json.dumps({'device': {'name': self.name,
            'ae_protocol_support_level': WallyAuthenticator.AE_SUPPORT_LEVEL,
            'supports_low_r': WallyAuthenticator.AE_SUPPORT_LEVEL == 0,
            'supports_liquid': 1,
    ...
    def _make_ae_signature(self, privkey, signing_hash, details):
        ...
        host_commitment = bytes.fromhex(details['host_commitment'])
        ...
        host_entropy = bytes.fromhex(details['host_entropy'])
    ...
    def sign_message(self, details: Dict) -> Dict:
        ...
        result = {}
        if details.get('use_ae_protocol'):
